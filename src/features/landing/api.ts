...
export const createLandingItem = async (type: string, data: FormData | LandingItem) => {
  const endpoint = type === 'processSteps' ? 'process-steps' : type;
  const res = await axiosSecure.post(`/landing/${endpoint}`, data);
  return res.data;
};

export const updateLandingItem = async (type: string, id: string, data: FormData | LandingItem) => {
  const endpoint = type === 'processSteps' ? 'process-steps' : type;
...
export const updateLandingConfig = async (data: FormData | Partial<LandingConfig>) => {
