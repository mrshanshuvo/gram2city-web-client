...
const getStatusBadge = (status: string) => {
  switch (status) {
    case "active":
      return <span className="px-2 py-0.5 rounded-full bg-green-100 text-green-700 text-[10px] font-bold uppercase">Active Hub</span>;
    case "limited":
      return <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[10px] font-bold uppercase">Express Only</span>;
    default:
      return <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[10px] font-bold uppercase">Launching Soon</span>;
  }
};

const CoverageMap: React.FC<CoverageMapProps> = ({ serviceCenters, targetCoords }) => {
            ...
            <Popup className="custom-popup">
              <div className="min-w-[200px] p-2 space-y-3">
                <div className="flex justify-between items-start">
                   <h3 className="font-black text-slate-900 text-lg leading-none">{district.city}</h3>
                   {getStatusBadge(district.status)}
                </div>
                
                <div className="space-y-1">
                  <p className="text-xs text-slate-500 font-medium italic">{district.district} District, {district.region}</p>
                </div>

                <div className="pt-2 border-t border-slate-100">
                  <p className="text-[11px] text-slate-600 font-bold mb-1 uppercase tracking-tight">Main Hub Zones:</p>
                  <div className="flex flex-wrap gap-1">
                    {district.covered_area.map((area, i) => (
                      <span key={i} className="text-[10px] bg-slate-50 px-2 py-0.5 rounded-md text-slate-500">{area}</span>
                    ))}
                  </div>
                </div>
