...
import { 
  FiPackage, FiMapPin, FiUser, FiClock, FiDollarSign, 
  FiArrowLeft, FiEdit, FiTrash2, FiActivity, FiCheckCircle 
} from "react-icons/fi";
  ...
  if (!parcel) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-500 mt-2">The shipment you are looking for does not exist or has been removed.</p>
        <button 
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          {parcel.delivery_status === "not_collected" && (
            <button 
              onClick={() => navigate(`/dashboard/editParcel/${parcel._id}`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 md:p-10">
            <div className="flex flex-col md:flex-row justify-between gap-6 mb-12">
              <div>
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Tracking Identity</span>
                <h1 className="text-3xl font-black text-gray-900 tracking-tighter font-mono">{parcel.trackingId}</h1>
              </div>
              <div className="flex flex-col md:items-end">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Current Status</span>
                <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest border ${statusColors[parcel.delivery_status] || "bg-gray-50 text-gray-500 border-gray-100"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative">
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="font-black text-gray-900 uppercase tracking-tight">Pickup Point</h3>
                </div>
                <div className="space-y-4 text-sm">
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Sender Name</p>
                  ...
                  <div>
                    ...
                    <p className="font-bold text-gray-600">{parcel.senderPhone || parcel.senderContact}</p>
                  </div>
                  <div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="font-black text-gray-900 uppercase tracking-tight">Delivery Hub</h3>
                </div>
                <div className="space-y-4 text-sm">
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Recipient Name</p>
                    <p className="font-black text-gray-800">{parcel.receiverName}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Contact</p>
                    <p className="font-bold text-gray-600">{parcel.receiverPhone || parcel.receiverContact || parcel.receiverPhoneNumber}</p>
                  </div>
                  <div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 md:p-10">
            ...
            {isTrackingLoading ? (
              ...
              <div className="space-y-0">
                {trackings.map((update: any, idx: number) => (
                  <div key={update._id} className="relative flex gap-8 pb-12 group last:pb-0">
                    ...
                    <div className={`relative z-10 w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-sm transition-all duration-500 ${
                      idx === 0 ? "bg-primary text-white scale-110 shadow-lg shadow-primary/20" : "bg-gray-50 text-gray-300"
                    }`}>
                    ...
                    <div className={`flex-1 pt-1 ${idx === 0 ? "opacity-100" : "opacity-60"}`}>
                      ...
                      <p className="text-sm text-gray-500 italic leading-relaxed">"{update.details}"</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="text-center py-10 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                ...
                <p className="text-gray-400 text-sm font-bold">Waiting for first tracking update...</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-[#1E5AA8] text-white rounded-[2.5rem] p-8 md:p-10 shadow-xl shadow-blue-900/20 relative overflow-hidden group">
            ...
            <span className="text-[10px] font-black text-blue-300 uppercase tracking-widest block mb-4">Financial Summary</span>
            ...
            <div className="space-y-6 border-t border-white/10 pt-8">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-blue-300 uppercase tracking-widest">Parcel Type</span>
                <span className="font-black text-xs uppercase">{parcel.parcelType}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-blue-300 uppercase tracking-widest">Gross Weight</span>
                <span className="font-black text-xs uppercase">{parcel.weight || parcel.parcelWeight} KG</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-blue-300 uppercase tracking-widest">Payment</span>
                <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase border ${
                  parcel.payment_status === "paid" ? "bg-emerald-500/20 text-emerald-300 border-emerald-500/30" : "bg-rose-500/20 text-rose-300 border-rose-500/30"
                }`}>
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-8 text-center">
             <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-6">Internal ID</span>
             <code className="text-xs font-bold text-gray-300 bg-gray-50 px-4 py-2 rounded-xl">{parcel._id}</code>
             <p className="text-[10px] text-gray-400 mt-6 font-medium">Booked on {moment(parcel.creation_date || parcel.createdAt).format("MMMM Do YYYY")}</p>
          </div>
        </div>
      </div>
    </div>
