          ...
          >
            <div className="px-8 py-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50 flex-shrink-0">
              <div>
                ...
                <p className="text-slate-500 text-sm font-medium">Hero section visual content management</p>
              </div>
              <button 
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6 overflow-y-auto">
                ...
                <div className="md:col-span-2">
                  ...
                  <div 
                    ...
                    className={`relative h-64 w-full rounded-3xl border-2 border-dashed transition-all cursor-pointer overflow-hidden flex flex-col items-center justify-center group ${
                      previewUrl ? "border-[#2E7D32]/30" : "border-slate-200 hover:border-[#2E7D32]/50 hover:bg-slate-50"
                    }`}
                  >
                    {previewUrl ? (
                      <>
                      ...
                      <div className="text-center space-y-2 p-8">
                        ...
                        <p className="text-slate-500 font-bold">Click to upload banner image</p>
                        <p className="text-slate-400 text-xs font-medium">PNG, JPG or WebP (Max 10MB)</p>
                      </div>
                    )}
                    ...
                    {uploading && (
                      <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center gap-3">
                         ...
                         <p className="text-[#2E7D32] font-black text-sm uppercase tracking-widest">Uploading to Cloud...</p>
                      </div>
                    )}
                  </div>
                  <input 
                  ...
                  {errors.image && <p className="text-xs text-red-500 font-bold mt-2 ml-1">{errors.image.message as string}</p>}
                </div>
              ...
              <div className="pt-6 flex justify-end gap-4 flex-shrink-0">
                ...
                >
                  {isLoading ? "Saving..." : <><Save size={20} /> Save Slide</>}
                </button>
              </div>
            </form>
          </motion.div>
