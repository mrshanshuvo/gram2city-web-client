...
const LandingPageManager = () => {
  ...
  if (bannersLoading || servicesLoading || configLoading || avatarsLoading || stepsLoading || featuresLoading || testimonialsLoading || partnersLoading) {
    return (
  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">Landing Page Manager</h1>
          <p className="text-slate-500 font-medium mt-1">100% Dynamic Content Control Center</p>
        </div>
      </div>
                  ...
                  <div key={banner._id} className="group relative bg-slate-50 rounded-3xl overflow-hidden border border-slate-200">
                    <div className="h-48 relative">
                      ...
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity gap-3">
                         <button onClick={() => setModalState({ type: "banners", data: banner })} className="p-3 bg-white rounded-full text-blue-500 hover:scale-110 transition-transform"><Edit3 size={20} /></button>
                         <button onClick={() => deleteMutation.mutate({ type: "banners", id: banner._id })} className="p-3 bg-white rounded-full text-red-500 hover:scale-110 transition-transform"><Trash2 size={20} /></button>
                      </div>
                    </div>
                    <div className="p-6">
                      <h3 className="font-black text-xl text-slate-900">{banner.title}</h3>
                      <p className="text-slate-500 text-sm font-medium line-clamp-2 mt-2">{banner.subtitle}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {services.map((service: any) => (
                  <div key={service._id} className="p-6 bg-slate-50 rounded-3xl border border-slate-200 flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 text-sm font-medium mt-1 line-clamp-2">{service.description}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {features.map((feature: any) => (
                  <div key={feature._id} className="p-6 bg-slate-50 rounded-3xl border border-slate-200 flex flex-col gap-4">
                    ...
                    <div className="flex justify-between items-start">
                       ...
                       <div className="flex gap-1">
                          <button onClick={() => setModalState({ type: "features", data: feature })} className="p-2 hover:bg-white rounded-xl text-slate-400 hover:text-blue-500"><Edit3 size={18} /></button>
                          <button onClick={() => deleteMutation.mutate({ type: "features", id: feature._id })} className="p-2 hover:bg-white rounded-xl text-slate-400 hover:text-red-500"><Trash2 size={18} /></button>
                       </div>
                    </div>
                  </div>
                ))}
          ...
          {activeTab === "testimonials" && (
            <div className="space-y-8">
              <div className="flex justify-between items-center">
                 ...
                 <AddButton label="Add Quote" onClick={() => setModalState({ type: "testimonials", data: null })} />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {testimonials.map((t: any) => (
                  <div key={t._id} className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-200 relative group">
                    <div className="absolute top-8 right-8 text-slate-200 group-hover:text-[#2E7D32]/20 transition-colors"><Quote size={40} /></div>
                    <div className="flex items-center gap-4 mb-6">
                       <img src={t.image} className="w-14 h-14 rounded-2xl object-cover shadow-sm" alt="" />
                       <div>
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-slate-200">
                       <div className="flex gap-1 text-[#F4C20D]">{[...Array(t.rating || 5)].map((_, i) => <Star key={i} size={14} fill="currentColor" />)}</div>
                       <div className="flex gap-1">
                          <button onClick={() => setModalState({ type: "testimonials", data: t })} className="p-2 hover:bg-white rounded-xl text-slate-400 hover:text-blue-500"><Edit3 size={18} /></button>
                          <button onClick={() => deleteMutation.mutate({ type: "testimonials", id: t._id })} className="p-2 hover:bg-white rounded-xl text-slate-400 hover:text-red-500"><Trash2 size={18} /></button>
                       </div>
                    </div>
                  </div>
                ))}
          ...
          {activeTab === "partners" && (
            <div className="space-y-8">
              <div className="flex justify-between items-center">
                 ...
                 <AddButton label="Add Partner" onClick={() => setModalState({ type: "partners", data: null })} />
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                {partners.map((p: any) => (
                  <div key={p._id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex flex-col items-center group relative">
                    <img src={p.logo} className="h-12 w-auto object-contain grayscale group-hover:grayscale-0 transition-all" alt={p.name} />
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter mt-4">{p.name}</p>
                    <div className="absolute top-2 right-2 flex opacity-0 group-hover:opacity-100 transition-opacity">
                       <button onClick={() => setModalState({ type: "partners", data: p })} className="p-1.5 hover:bg-white rounded-lg text-blue-500"><Edit3 size={14} /></button>
                       <button onClick={() => deleteMutation.mutate({ type: "partners", id: p._id })} className="p-1.5 hover:bg-white rounded-lg text-red-500"><Trash2 size={14} /></button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {processSteps.map((step: any) => (
                  <div key={step._id} className="p-6 bg-slate-50 rounded-3xl border border-slate-200">
                    <div className="flex justify-between items-start mb-4">
                       <div className="w-10 h-10 bg-[#2E7D32] text-white rounded-xl flex items-center justify-center font-black">{step.order}</div>
                       <div className="flex gap-1">
                          <button onClick={() => setModalState({ type: "processSteps", data: step })} className="p-2 hover:bg-white rounded-xl text-slate-400 hover:text-blue-500"><Edit3 size={18} /></button>
                          <button onClick={() => deleteMutation.mutate({ type: "processSteps", id: step._id })} className="p-2 hover:bg-white rounded-xl text-slate-400 hover:text-red-500"><Trash2 size={18} /></button>
                       </div>
                    </div>
            ...
            <div className="space-y-8">
              <div className="flex justify-between items-center bg-slate-50 p-6 rounded-3xl border border-slate-100">
                <div>
                  ...
                  <p className="text-slate-500 font-medium text-sm">Magic generation for user profiles</p>
                </div>
                <button onClick={() => generateAvatarsMutation.mutate()} className="btn bg-[#F4C20D] hover:bg-[#EBC00D] text-slate-900 rounded-2xl font-black gap-2">
                  <Wand2 size={20} /> Magic Generate
                </button>
              </div>
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-6">
                {avatars.map((a: any) => (
                  <div key={a._id} className="relative group bg-slate-50 p-3 rounded-2xl border border-slate-100 flex flex-col items-center">
                    <img src={a.url} className="w-16 h-16 rounded-xl shadow-sm" alt="" />
                    <button onClick={() => deleteAvatarMutation.mutate(a._id)} className="absolute -top-2 -right-2 p-1.5 bg-white rounded-lg text-red-500 opacity-0 group-hover:opacity-100 shadow-md"><Trash2 size={12} /></button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="max-w-4xl">
              <form onSubmit={handleSaveConfig} className="space-y-12">
                <div className="space-y-6">
                  ...
                  <div className="space-y-4 bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                    <div className="space-y-2">
                       ...
                       <input name="title" defaultValue={config?.merchantSection?.title} className="w-full p-4 rounded-2xl border border-slate-200 focus:ring-4" />
                    </div>
                    <div className="space-y-2">
                       <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Description</label>
                       <textarea name="description" rows={3} defaultValue={config?.merchantSection?.description} className="w-full p-4 rounded-2xl border border-slate-200 focus:ring-4" />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                    <div className="md:col-span-2 space-y-2">
                       ...
                       <input name="address" defaultValue={config?.contactInfo?.address} className="w-full p-4 rounded-2xl border border-slate-200" />
                    </div>
                    <div className="space-y-2">
                       <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Phone</label>
                       <input name="phone" defaultValue={config?.contactInfo?.phone} className="w-full p-4 rounded-2xl border border-slate-200" />
                    </div>
                    <div className="space-y-2">
                       <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Email</label>
                       <input name="email" defaultValue={config?.contactInfo?.email} className="w-full p-4 rounded-2xl border border-slate-200" />
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                    {["Twitter", "Facebook", "LinkedIn", "Instagram", "YouTube"].map(s => (
                      <div key={s} className="space-y-2">
                         <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">{s}</label>
                ...
                <div className="flex justify-end">
                  <button type="submit" disabled={updateConfigMutation.isPending} className="px-12 py-5 bg-[#2E7D32] text-white rounded-2xl font-black shadow-xl flex items-center gap-2">
      ...
      {/* Modals */}
      <BannerModal isOpen={modalState.type === "banners"} onClose={() => setModalState({ type: null, data: null })} onSubmit={handleModalSubmit} initialData={modalState.data} isLoading={createMutation.isPending || updateMutation.isPending} />
      <ServiceModal isOpen={modalState.type === "services"} onClose={() => setModalState({ type: null, data: null })} onSubmit={handleModalSubmit} initialData={modalState.data} isLoading={createMutation.isPending || updateMutation.isPending} />
      <ProcessStepModal isOpen={modalState.type === "processSteps"} onClose={() => setModalState({ type: null, data: null })} onSubmit={handleModalSubmit} initialData={modalState.data} isLoading={createMutation.isPending || updateMutation.isPending} />
      <FeatureModal isOpen={modalState.type === "features"} onClose={() => setModalState({ type: null, data: null })} onSubmit={handleModalSubmit} initialData={modalState.data} isLoading={createMutation.isPending || updateMutation.isPending} />
      <TestimonialModal isOpen={modalState.type === "testimonials"} onClose={() => setModalState({ type: null, data: null })} onSubmit={handleModalSubmit} initialData={modalState.data} isLoading={createMutation.isPending || updateMutation.isPending} />
      <PartnerModal isOpen={modalState.type === "partners"} onClose={() => setModalState({ type: null, data: null })} onSubmit={handleModalSubmit} initialData={modalState.data} isLoading={createMutation.isPending || updateMutation.isPending} />
    </div>
  );
};
