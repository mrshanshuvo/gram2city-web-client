...
const UpdateProfile = () => {
  ...
  if (userLoading) return <div className="flex items-center justify-center h-96"><Loader2 className="animate-spin text-primary" size={40} /></div>;
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-20">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mt-1">Manage your platform persona</p>
        </div>
        {dbUser?.isProfileComplete && (
           <div className="flex items-center gap-2 px-6 py-2.5 bg-[#2E7D32]/10 text-[#2E7D32] rounded-2xl border border-[#2E7D32]/20 shadow-sm">
             ...
             <span className="font-black uppercase tracking-widest text-[10px]">Verified Shipper</span>
           </div>
        )}
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm text-center relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <div className="mt-8">
                <h3 className="text-2xl font-black text-slate-800 tracking-tighter">{user?.displayName}</h3>
              ...
              <div className="mt-10 pt-10 border-t border-slate-50 flex justify-center gap-8">
                <div>
                  <p className="text-2xl font-black text-slate-800">{dbUser?.isProfileComplete ? "100%" : "65%"}</p>
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Trust Tier</p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-slate-900 rounded-[3rem] p-8 text-white shadow-2xl relative overflow-hidden">
             ...
             <p className="text-xs font-bold text-slate-400 leading-relaxed relative z-10">
               Keep your phone and address updated to receive faster delivery assignments and verification badges.
             </p>
            ...
            {showAvatarGrid ? (
              ...
              >
                <div className="flex justify-between items-center mb-8">
                  ...
                  <button onClick={() => setShowAvatarGrid(false)} className="text-xs font-black text-blue-600 hover:underline">BACK TO FORM</button>
                </div>
                <div className="grid grid-cols-4 md:grid-cols-6 gap-4">
                  {avatarLibrary.map((avatar: any, i: number) => (
                    <button
                      ...
                      className={`relative aspect-square rounded-2xl overflow-hidden border-4 transition-all hover:scale-105 active:scale-95 ${
                        currentPhotoURL === avatar.url ? 'border-blue-500 shadow-lg' : 'border-slate-50'
                      }`}
                    >
                  ...
                  <div className="aspect-square rounded-2xl bg-slate-50 border-4 border-dashed border-slate-200 flex flex-col items-center justify-center text-center p-2">
                    ...
                    <span className="text-[8px] font-black text-slate-400 uppercase leading-tight">More Coming</span>
                  </div>
                </div>
              </motion.div>
            ) : (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      ...
                      <div className="relative group">
                        <UserIcon className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-600 transition-colors" size={20} />
                        <input
                          type="text"
                      ...
                      {errors.name && <span className="text-[10px] text-red-500 font-bold ml-2">{(errors.name.message as string)}</span>}
                    </div>

                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Platform Email</label>
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Custom Avatar URL</label>
                      <div className="relative group">
                        <Camera className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-600 transition-colors" size={20} />
                        <input
                          type="url"
                    ...
                    <div className="space-y-3 md:col-span-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Base Pickup Address</label>
                      <div className="relative group">
                        <MapPin className="absolute left-5 top-7 text-slate-300 group-focus-within:text-blue-600 transition-colors" size={20} />
                        <textarea
