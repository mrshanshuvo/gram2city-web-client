...
const FinancialSettings: React.FC = () => {
  ...
  const payoutStatusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const res = await axiosSecure.patch(`/admin/payouts/${id}/status`, { status });
      return res.data;
    },
    onSuccess: () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      ...
      <div className="space-y-6 pt-8 border-t border-gray-100">
        <div>
          <h3 className="text-2xl font-black text-gray-800 tracking-tight">Rider Payout Requests</h3>
          <p className="text-gray-500 font-medium">Review and process mission earnings withdrawal requests.</p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-100">
              <thead className="bg-gray-50/50">
                <tr>
                  <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Rider</th>
                  <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Amount</th>
                  <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Requested At</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {payouts.map((payout: any) => (
                  <tr key={payout._id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-[10px] text-gray-400 font-medium">{payout.rider_email}</div>
                    </td>
                    <td className="px-6 py-4 text-sm font-black text-emerald-600">৳{payout.amount}</td>
                ...
                {payouts.length === 0 && (
                  <tr>
                    <td colSpan={5} className="py-12 text-center text-gray-400 font-bold italic text-sm">No pending payout requests.</td>
                  </tr>
                )}
              </tbody>
            </table>
