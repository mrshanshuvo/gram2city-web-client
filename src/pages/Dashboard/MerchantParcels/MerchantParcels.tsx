...
const MerchantParcels = () => {
  ...
  const bulkMutation = useMutation({
    mutationFn: async (parcels: any[]) => {
      const res = await axiosSecure.post("/parcels/bulk", { parcels, merchantId: user?._id });
      return res.data;
    },
    onSuccess: () => {
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-100">
            <thead className="bg-slate-50/50">
              <tr>
                <th className="px-8 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Parcel & ID</th>
                <th className="px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Customer</th>
                <th className="px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Destination</th>
                <th className="px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">COD Amount</th>
            ...
            <tbody className="bg-white divide-y divide-slate-50">
              {filteredParcels.map((parcel: Parcel) => (
                <tr key={parcel._id} className="hover:bg-slate-50/30 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">#{parcel.trackingId || parcel._id.slice(-8)}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-6">
                    ...
                    <div className="text-[10px] text-slate-400 font-medium">{parcel.receiverContact}</div>
                  </td>
                  <td className="px-6 py-6">
                    <div className="text-xs font-black text-slate-600 truncate max-w-[150px]">{parcel.deliveryAddress}</div>
                    <div className="text-[10px] font-bold text-blue-500 uppercase">{parcel.receiverDistrict}</div>
                  </td>
                  <td className="px-6 py-6">
                    <div className="text-sm font-black text-emerald-600">৳{parcel.codAmount || 0}</div>
                    <div className="text-[10px] font-bold text-slate-400 uppercase">Collection</div>
                  </td>
                  <td className="px-6 py-6">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                      parcel.delivery_status === "delivered" ? "bg-emerald-50 text-emerald-600" :
                      parcel.delivery_status === "on_the_way" ? "bg-blue-50 text-blue-600" :
              ...
              {filteredParcels.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-gray-400 font-bold italic">No shipments found matching your criteria.</td>
                </tr>
              )}
            </tbody>
          </table>
            ...
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">Upload your CSV and map fields</p>
                </div>
              </div>
              <button onClick={() => setIsBulkModalOpen(false)} className="text-slate-300 hover:text-slate-600 transition-colors">
                <FiXCircle size={32} />
              </button>
            </div>
            ...
            {!csvData.length ? (
              <div className="flex-1 flex flex-col items-center justify-center border-4 border-dashed border-slate-50 rounded-[2rem] p-20 text-center space-y-6">
                ...
                <div>
                  ...
                  <p className="text-slate-400 font-medium text-sm">Download our template to ensure correct formatting</p>
                </div>
                <input
                  type="file"
                  accept=".csv"
                ...
                >
                  {isParsing ? <span className="loading loading-spinner loading-xs"></span> : <FiUpload />}
                  Choose File
                </label>
              </div>
            ) : (
