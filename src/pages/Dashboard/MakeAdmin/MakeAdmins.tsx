...
const MakeAdmins = () => {
  ...
  if (authLoading) return <div className="p-20 text-center"><span className="loading loading-spinner loading-lg text-primary"></span></div>;
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: "Super Admins", count: summary?.superAdmin, icon: FiStar, color: "indigo" },
          { label: "Platform Staff", count: summary?.admin, icon: FiUserCheck, color: "emerald" },
          { label: "Total Fleet", count: summary?.total, icon: FiUsers, color: "blue" },
          { label: "Active Today", count: summary?.activeToday, icon: FiActivity, color: "amber" },
        ].map((card, i) => (
          <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-4 group hover:shadow-xl transition-all">
            <div className={`w-12 h-12 bg-${card.color}-50 text-${card.color}-600 rounded-2xl flex items-center justify-center text-xl group-hover:scale-110 transition-transform`}>
              <card.icon />
            </div>
            <div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="avatar placeholder">
                        <div className="bg-slate-100 text-slate-400 rounded-2xl w-12 h-12 font-black text-sm">
                          {user.photoURL ? <img src={user.photoURL} alt="User" /> : (user.displayName?.[0] || user.name?.[0] || user.email[0]).toUpperCase()}
                        </div>
                      </div>
                      <div>
                        <div className="font-black text-slate-800 text-sm">{user.displayName || user.name || "Anonymous User"}</div>
                        <div className="text-[10px] text-slate-400 font-bold font-mono">{user.email}</div>
                      </div>
                    </div>
                  </td>
                  <td>
                    <div className="flex flex-col gap-1">
                      <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest w-fit ${
                        ...
                        user.role === 'merchant' ? 'bg-purple-50 text-purple-600' :
                        user.role === 'rider' ? 'bg-amber-50 text-amber-600' :
                        ...
                        <button
                          onClick={() => {
                            ...
                            Swal.fire({
                              ...
                              if (res.isConfirmed) mutateStatus({ email: user.email, status: newStatus });
                            });
                          }}
                          className={`btn btn-xs border-none font-black uppercase tracking-tight rounded-lg ${
                            user.status === 'suspended' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                          }`}
                        >
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            Swal.fire({
                              ...
                              if (res.isConfirmed) mutateRole({ email: user.email, role: newRole });
                            });
                          }}
                        >
