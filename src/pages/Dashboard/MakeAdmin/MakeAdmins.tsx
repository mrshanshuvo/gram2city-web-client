...
const MakeAdmins = () => {
  ...
  if (authLoading) return <div className="flex justify-center p-20"><span className="loading loading-spinner loading-lg text-primary"></span></div>;
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Role Hierarchy & Permissions</p>
          </div>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Super Admins</p>
      ...
      <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-8 py-6 border-b border-gray-50 flex justify-between items-center bg-gray-50/30">
          ...
          <span className="badge badge-primary badge-sm font-bold py-3 px-4">{displayUsers.length} Users</span>
        </div>
        ...
        <div className="overflow-x-auto">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="avatar placeholder">
                        <div className="bg-gray-100 text-gray-400 rounded-xl w-10 h-10 font-bold text-xs">
                          {user.photoURL ? <img src={user.photoURL} alt="User" /> : (user.name?.[0] || user.email[0]).toUpperCase()}
                        </div>
                      </div>
                      <div>
                        <div className="font-black text-gray-800 text-sm">{user.name || "Anonymous User"}</div>
                        <div className="text-[10px] text-gray-400 font-bold font-mono tracking-tighter">{user.email}</div>
                      </div>
                    </div>
                  </td>
                  <td>
                          ...
                          <button
                            ...
                            onClick={() => {
                              Swal.fire({
                                ...
                                if (res.isConfirmed) updateRole({ email: user.email, role: "user" });
                              });
                            }}
                          ...
                          <button
                            ...
                            onClick={() => {
                              Swal.fire({
                                ...
                                if (res.isConfirmed) updateRole({ email: user.email, role: "admin" });
                              });
                            }}
          ...
          {displayUsers.length === 0 && (
            <div className="p-10 text-center space-y-3">
              ...
              <p className="text-gray-400 font-bold text-sm tracking-tight">No staff members found matching your search.</p>
            </div>
          )}
        </div>
      </div>
