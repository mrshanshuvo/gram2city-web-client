...
const ProfileCompletionTracker: React.FC<TrackerProps> = ({ user }) => {
  const steps = [
    ...
    { id: "address", label: "Pickup Address", isDone: !!user?.address, icon: MapPin },
  ];
  ...
  if (percentage > 33) { statusText = "In Transit"; statusColor = "bg-blue-500"; }
  if (percentage === 100) { statusText = "Verified Shipper"; statusColor = "bg-[#2E7D32]"; }
    ...
    >
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8">
        <div className="space-y-4 text-center lg:text-left">
          <div className="flex items-center gap-2 justify-center lg:justify-start">
             <span className={`${statusColor} text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full`}>
               {statusText}
             </span>
        ...
        <div className="relative flex flex-col items-center gap-4">
          ...
          {percentage < 100 && (
             <Link to="/dashboard/update-profile" className="flex items-center gap-1 text-sm font-black text-[#2E7D32] hover:gap-2 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-10">
        {steps.map((step, idx) => (
          ...
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${step.isDone ? 'bg-[#2E7D32] text-white' : 'bg-slate-100 text-slate-400'}`}>
              <step.icon size={20} />
            </div>
            <div className="flex-grow">
              <p className="text-xs font-black text-slate-400 uppercase tracking-tighter">Step {idx + 1}</p>
              <h4 className={`text-sm font-bold ${step.isDone ? 'text-slate-900' : 'text-slate-500'}`}>{step.label}</h4>
            </div>
            {step.isDone ? (
